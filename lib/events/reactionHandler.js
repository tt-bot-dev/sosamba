...
class ReactionMenuHandler extends Event {
    ...
    prerequisites(msg, emoji, userID) {
        ...
        if (emoji.name !== STOP_BUTTON && !menu.callbacks.hasOwnProperty(emoji.id ? `${emoji.name}:${emoji.id}` : emoji.name)) return false;
    ...
    async run(msg, emoji) {
        ...
        if (this.sosamba.hasBotPermission(msg.channel, "manageMessages")) msg.removeReaction(emote, menu.user);
