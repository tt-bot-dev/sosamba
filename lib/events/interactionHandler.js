...
const { Constants: { ButtonStyles, ComponentTypes, InteractionTypes, ApplicationCommandOptionTypes } } = require("eris");
...
class InteractionCreateHandler extends Event {
    ...
    _getFriendlyCommandOption(opts) {
        ...
        return opts.map(opt => {
            return `${opt.required != null ? opt.required ? "<" : "[" : ""}${opt.name}: ${opt.choices ? 
                opt.choices.map(o => o.name).join("|") :
                this._getFriendlyCommandTypeName(opt.type)}${opt.required != null ? opt.required ? ">" : "]" : ""}`
        }).join(" ");
    }
