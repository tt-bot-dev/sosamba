...
const { Constants: { InteractionTypes } } = require("@projectdysnomia/dysnomia");
...
class ComponentAwaiter extends InteractionListener {
    ...
    async run(ctx) {
        ...
        let isAny = false, d = this.anyListeners.get(interactionID);
        if (d) isAny = true;
        ...
        d.rs(ctx);
        if (isAny) {
            this.anyListeners.delete(interactionID);
        } else {
            this.listeners.delete(`${interactionID}:${ctx.author.id}`);
        }
    }
