...
class MessageCreateHandler extends Event {
    ...
    async run(msg) {
        ...
        const prefix = arrayStartsWith(msg.content, p, true);
        if (prefix) this.handleCommand(context, prefix);
    ...
    async handleCommand(ctx, prefix) {
        ...
        if (!cmd) cmd = this.sosamba.commands.find(c => c.aliases.some(a => a.toLowerCase() === command.toLowerCase()));
        if (!cmd) {
